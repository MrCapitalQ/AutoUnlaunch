    ...
    : LauncherProcessTrackingHandler(processWatcher, epicSettingsService, timeProvider, logger)
{
    ...
    protected override Task StartCoreAsync(CancellationToken cancellationToken = default)
        => UpdateInstallPathsAsync(cancellationToken);
    ...
    private async Task UpdateInstallPathsAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (string.IsNullOrEmpty(manifestsDirectoryPath) || !Path.Exists(manifestsDirectoryPath))
            {
                _logger.LogWarning("Could not get Epic manifest directory path.");
                return;
            }

            foreach (var manifestPath in Directory.EnumerateFiles(manifestsDirectoryPath, "*.item"))
            {
                try
                {
